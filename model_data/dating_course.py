...
def third_dating_course(mem_id,top_n):
    if (df_ymti[:top_n].mem_sex.values=="f") & (df_ymti[:top_n].mbti_code.values[0][0]=="E"):
            return df_outside['outside'][2], df_outside['url'][2]
    elif (df_ymti[:top_n].mem_sex.values=="f") & (df_ymti[:top_n].mbti_code.values[0][0]=="I"):
...
def third_food_course(mem_id,top_n):
    me = df_2030[df_2030['mem_no']==mem_id].food_cat.values[0].split()
    person = df_ymti[:top_n].food_cat.values[0].split()
    a = list(set(me).intersection(person))
    random.shuffle(a)
    if a[0] =="western":
